...
import com.example.asmagile.ViewHolder.ViewHolder_Customer;
...
public class KhachHangActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerView_Customer =findViewById(R.id.recyclerView_Customer);
        fab_addCustomer = findViewById(R.id.fab_addCustomer);
        userDatabase = FirebaseDatabase.getInstance().getReference("User");
        fab_addCustomer.setOnClickListener(v -> show(0));
        recyclerView_Customer.setHasFixedSize(true);
        recyclerView_Customer.setLayoutManager(new LinearLayoutManager(KhachHangActivity.this, RecyclerView.VERTICAL, false));
                    ...
                    builder.setMessage("Bạn có chắc chắn xóa khách hàng này không?")
                            ...
                            .setPositiveButton("Có", (dialog, which) -> {
                                ...
                                reference.child(adapter.getRef(holder.getAbsoluteAdapterPosition()).getKey())
                                        .removeValue((error, ref) -> Toast.makeText(KhachHangActivity.this, "Xóa khách hàng thành công", Toast.LENGTH_SHORT).show());
                            }).create();
                    builder.show();
                });
            }
            ...
            public void onClick(View v) {
                if (validate() > 0) {
                    if (type == 0) {
                        ...
                        userDatabase.addValueEventListener(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                if (snapshot.hasChild(phone)) {
                                    Toast.makeText(KhachHangActivity.this, "Số điện thoại này đã được đăng ký", Toast.LENGTH_SHORT).show();
                                } else {
                                    user = new Users(name, pass, address,1, "");
                                    userDatabase.child(phone).setValue(user, (error, ref) -> Toast.makeText(KhachHangActivity.this, "Thêm thông tin khách hàng thành công", Toast.LENGTH_SHORT).show());
                        ...
                        userDatabase.child(user.getPhone()).updateChildren(map, (error, ref) -> {Toast.makeText(KhachHangActivity.this, "Cập nhập thông tin khách thành công", Toast.LENGTH_SHORT).show();alertDialog.dismiss();});
                    }
                }
            }
        });
